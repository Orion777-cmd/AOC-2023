...
func main(){
	fmt.Println("Day 13 Part 1 Result: ", ReflectionSummarizingAllNotes())
	fmt.Println("Day 13 Part 2 Result: ", ReflectionLineOnEachPattern())
}


func ReflectionLineOnEachPattern() int {
	b, _ := os.ReadFile("inputData.txt")
	input := string(b)
	grids := parse(input)
	res := 0
	for i := range grids {
